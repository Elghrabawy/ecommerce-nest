...
import { PaymentStatus, PaymentMethod, PaymentProvider } from 'src/common/utils/enums';
...
export const PaymentFactory = setSeederFactory(Payment, (faker: Faker) => {
  ...
  if (faker.datatype.boolean(0.1)) {
    payment.refundedAmount = parseFloat(faker.commerce.price({ min: 10, max: 100 }));
  }
  ...
  if ([PaymentStatus.COMPLETED, PaymentStatus.FAILED].includes(payment.status)) {
